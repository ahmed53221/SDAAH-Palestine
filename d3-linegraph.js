...
d3.linegraph = function(noTicks, noDots, parties, partyColors, partyNames, dataMax, dataMin, additionalMonths) {
    /* params */
    if (!parties) {
        parties = ['fatah', 'ppp', 'pflp', 'dflp', 'pni', 'twa', 'hamas', 'other'];
    }
    if (!partyColors) {
        partyColors = {'fatah': '#cce207', 'ppp': '#8B0000', 'pflp': '#ff0000', 'dflp': '#fffff', 'pni': '#ff8800', 'twa': '#c90cb9', 'hamas': '#ffffff', 'other': '#005507'};
    }
    if (!partyNames) {
        partyNames = {'fatah': 'fatah', 'ppp': 'ppp', 'pflp': 'pflp', 'dflp': 'dflp + Bani', 'pni': 'pni', 'twa': 'twa', 'hamas': 'hamas', 'other': 'others'};
    }
    if (!additionalMonths) {
        additionalMonths = 10;
    }
    ...
    function linegraph(dataset) {
     dataset.each(function (data) {
      ...
      if (!dataMax) {
          const maxfatah = d3.max(data, d => d.fatah);
          const maxhamas = d3.max(data, d => d.hamas);
          dataMax = maxfatah >= maxhamas ? maxfatah + 10 : maxhamas + 10;
          dataMin = 0;
      }
